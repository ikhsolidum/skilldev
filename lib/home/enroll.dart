...
class Course {
  ...
  factory Course.fromJson(Map<String, dynamic> json) {
    return Course(
      ...
      archived: json['archived'] == true || json['archived'] == 1, // Ensure boolean check
    );
  }
}
...
class _EnrollmentPageState extends State<EnrollmentPage> {
  ...
  List<String> _categories = ['All', 'Development', 'Design', 'IT', 'Marketing'];
  ...
  Future<void> _loadCourses() async {
    try {
      ...
      setState(() {
        _courses = courses.where((course) => !course.archived).toList(); // Filter archived courses
        _isLoading = false;
      });
    } catch (e) {
  ...
  try {
    final coursesResponse = await http.get(
      Uri.parse('http://bunn.helioho.st/courses.php?userId=${widget.userId}')
    );
    ...
    if (coursesResponse.statusCode == 200 && enrolledResponse.statusCode == 200) {
      final coursesData = jsonDecode(coursesResponse.body) as Map<String, dynamic>;
      final enrolledData = jsonDecode(enrolledResponse.body) as Map<String, dynamic>;

      if (coursesData['success']) {
        final enrolledIds = enrolledData['success'] 
          ? (enrolledData['courses'] as List).map((c) => c['id'].toString()).toSet()
          : <String>{};

        return (coursesData['courses'] as List)
          .where((c) => !enrolledIds.contains(c['id'].toString()))
          .map((c) => Course.fromJson(c))
          .where((course) => !course.archived) // Add this filter
          .toList();
      }
    }
